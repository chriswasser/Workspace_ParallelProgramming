...

{
  ...
  #pragma omp for private(j, i, resid) reduction(+:error)
    for (j=1; j<m-1; j++){
      for (i=1; i<n-1; i++){
        resid =(
...
#pragma omp parallel for private(j, i, xx, yy)
  for (j=0; j<m; j++){
    for (i=0; i<n; i++){
      xx = -1.0 + *dx * (i-1);
      yy = -1.0 + *dy * (j-1);
                 ...
                 double *f)
{
  ...
  dy = 2.0 / (m-1);
  error = 0.0;
#pragma omp parallel for private(j,i,xx,yy,temp) reduction(+:error)
  for (j=0; j<m; j++){
    for (i=0; i<n; i++){
      xx = -1.0 + dx * (i-1);
      yy = -1.0 + dy * (j-1);
