...
void serialQuicksort(int * array, int first, int last){
	int pivotElement;
	if(first < last) {
		pivotElement = pivot(array,first,last);
		serialQuicksort(array,first, pivotElement-1);
		serialQuicksort(array,pivotElement+1,last);
	}
}
...
void quicksort(int * array, int first, int last){
	...
	if(last-first > 10000){
		pivotElement = pivot(array,first,last);
	#pragma omp task shared(array, pivotElement, first)
	{
		quicksort(array,first,pivotElement-1);
	}
	#pragma omp task shared(array, pivotElement, last)
	{
		quicksort(array,pivotElement+1,last);
	}
	#pragma omp taskwait
	} else {
		serialQuicksort(array, first, last);
	}
}
