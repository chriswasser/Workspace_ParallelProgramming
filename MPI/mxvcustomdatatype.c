...
int main (int argc, char* argv[])
{
	...
	if(myRank == 0) {
		...
		height = height / nProcs; // height now accounts for the number of rows each process calculates
	}
	...
	MPI_Type_contiguous(width, MPI_DOUBLE, &row_type);
	...
	// Bcast x
	MPI_Bcast(vec, 1, row_type, 0, MPI_COMM_WORLD);
	// Scatter A
	MPI_Scatter(A, height, row_type, mat, height, row_type, 0, MPI_COMM_WORLD);
